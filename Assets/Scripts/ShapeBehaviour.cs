...
{
    [HideInInspector] public bool tutorialPlaced = false; // Håller koll på om blocket lagts i tutorialen
    ...
    // --- NY METOD: Tvinga blocket att starta som OSLEPPT ---
    private void Awake()
    {
        tutorialPlaced = false;
    }
    ...
    public void Initialize(Shape shape, Color[] colors, GameObject prefab)
    {
        ...
        // Extra säkerhetsåtgärd vid initiering
        tutorialPlaced = false; 
    }
   ...
   private void UpdateGhost()
    ...
    void OnMouseUp()
    {
        ...
        if (valid)
        {
            ...
            // --- FIXAD OCH SÄKRAD LOGIK FÖR BÅDA SCENERNA ---
            if (TutorialBlockSpawner.Instance != null)
            {
                tutorialPlaced = true; // Säg till TutorialManager att detta block är klart!
                TutorialBlockSpawner.Instance.BlockPlaced();
            }
            else if (BlockSpawner.Instance != null)
            {
                // Denna måste ligga här så att vanliga MainGame kan spawna block!
